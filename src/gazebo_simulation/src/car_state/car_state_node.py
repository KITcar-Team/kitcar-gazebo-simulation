...
class CarStateNode:
    ...
    def __init__(self):
        ...
        car_config = rospy.get_param(
            "~car_config",
            os.environ.get("KITCAR_REPO_PATH") + "/kitcar-gazebo-simulation/models/model_db/dr_drift/dr_drift.yaml",
        )
    ...
    def start(self):
        ...
        self.publisher = rospy.Publisher(Topic.STATE, CarStateMsg, queue_size=1)

        # Start subscribing to changes in gazebo
        self.subscriber = rospy.Subscriber("/gazebo/model_states", ModelStates, callback=self.model_state_cb)
