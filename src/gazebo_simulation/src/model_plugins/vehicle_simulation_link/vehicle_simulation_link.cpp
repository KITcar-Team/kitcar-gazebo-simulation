...
void VehicleSimulationLink::simulationWorldTransformation(const geometry_msgs::TransformStamped wv_stamped){
	...
	ignition::math::Pose3d wv_pose(wv_tf.translation.x, wv_tf.translation.y,wv_tf.translation.z, wv_tf.rotation.w, wv_tf.rotation.x, wv_tf.rotation.y, wv_tf.rotation.z);   
	ignition::math::Pose3d vw_pose = wv_pose.Inverse();

	// Simulation to world == sim to vehicle -> vehicle to world 
	// This transformation works (Confused why tho...): it seems like vehicle to world + vehicle to sim
