...
class StateMachine:
    ...
    This class handels state changes for a StateMachine.
    It also adds the methods's info, value and msg which return
    properties of the current state.
    Additionally, it defines a property which returns all states and a method which
    creates a graph with all states.
    ...
    def callback_on_state_change(func):
        """Decorator which executes self.callback when the state of state machine has
        changed.
    ...
    def all_states(self) -> Dict[int, State]:
        """Property which gives all available states inherting from State in current
        StateMachine."""
    ...
    def generate_graph(
        ...
        .. note::
            You can find documentation on graphviz on their
            `homepage <https://graphviz.org/documentation/>`_ and on `readthedocs
            <https://graphviz.readthedocs.io/en/stable/manual.html#basic-usage>`_.
        """
        ...
        for _, state in self.all_states.items():
            for msg, index in msgs:
                ...
                try:
                    ...
                    if next_state != state:
                        if child.startswith("Failure"):
                            ...
                            # FIXME: A state that does not receive a message doesn't
                            # gets generate this way. Instead, he gets generated
                            # by creating an edge (of other node)
                            # -> Therefore he gets no styling
                            g.node(
                                child,
                                label=child,
                                shape=shape,
                ...
                # Is thrown in states used for ProgressStateMachine when something
                # unexpected happened
