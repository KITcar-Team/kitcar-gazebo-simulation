...
import math
from road_generation import utils
from os import path
import os
import hashlib
...
class MarkerImage(Enum):
    TURN_LEFT = 'road_generation/renderer/street_markings/Fahrbahnmarkierung_Pfeil_L.svg'
    TURN_RIGHT = 'road_generation/renderer/street_markings/Fahrbahnmarkierung_Pfeil_R.svg'
...
def draw_road_marking(ctx, marking, generator_dir):
    ...
    if marking_visual.marker_image:
        ctx.save()
        handle = Rsvg.Handle()
        svg = handle.new_from_file(path.join(generator_dir,marking_visual.marker_image.value))
...
def draw_start_lane(ctx, lanelet):
    left = boundary_to_equi_distant(lanelet.leftBoundary, 0.02, 0.01)
    right = boundary_to_equi_distant(lanelet.rightBoundary, 0.02, 0.01)
    ...
    #ctx.save()
        ...
        ctx.save()
        for (l, r) in iter:
            lx = l[0] + idx/3.0*(r[0]-l[0])
            ly = l[1]
            rx = l[0] + (idx+1.0)/3.0*(r[0]-l[0])
            ry = r[1]
...
def draw(doc, generator_dir, road_name, tile_size = 4, include_empty=True, segmentation = False):
    ...
    materials_dir = os.path.join(os.environ.get('KITCAR_REPO_PATH'),'kitcar-gazebo-simulation','simulation/models/env_db',road_name,'materials')
    ...
    #Adjust bounding box to fit number of tiles and position y=0 in middle of a tile
    bounding_box.x_min = math.floor(bounding_box.x_min*PIXEL_PER_UNIT/tile_size)*tile_size/PIXEL_PER_UNIT #- 0.5 * tile_size/PIXEL_PER_UNIT
    bounding_box.y_min = math.floor(bounding_box.y_min*PIXEL_PER_UNIT/tile_size)*tile_size/PIXEL_PER_UNIT - 0.5 * tile_size/PIXEL_PER_UNIT
    bounding_box.x_max = math.ceil(bounding_box.x_max*PIXEL_PER_UNIT/tile_size)*tile_size/PIXEL_PER_UNIT #+ 0.5 * tile_size/PIXEL_PER_UNIT
    bounding_box.y_max = math.ceil(bounding_box.y_max*PIXEL_PER_UNIT/tile_size)*tile_size/PIXEL_PER_UNIT + 0.5 * tile_size/PIXEL_PER_UNIT
       ...
       #Get hash of empty tile
        ...
        for lanelet in doc.lanelet:
            ...
            if lanelet.type == "zebraCrossing":
                ctx.set_source_rgb(*color_classes.rgb(color_classes.ColorClass.ZEBRA_CROSSING))
...
def ground_plane_model(x, y, tile_size, name, material, road_name):
    ...
    """.format(x=x, y=y, tile_size=tile_size, name=name, road_name=road_name, material=material)
