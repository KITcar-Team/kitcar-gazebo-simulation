...
class Polygon(shapely.geometry.polygon.Polygon):
    ...
    def to_schema_lanelet(self, split_idx=0):
        """
        Conversion to a schema.lanelet. This is done by splitting the polygon in two parts. The first @split_idx- number of points are considered to be on the right side.
        The returned lanelet has no line markings. 
        """
        if split_idx == 0:
    ...
    def to_geometry_msg(self):
        ...
        msg.points = [p.to_geometry_msg() for p in self.get_points()]
        return msg
